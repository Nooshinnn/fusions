...
OUT_DIR = Path("phase04_cross_gated_fusion_outputs")
...
# DATASET
# ============================================================
class CrossGatedDataset(Dataset):
...
class CrossGatedFusion(nn.Module):
    def __init__(self, tab_dim, url_dim, html_dim, hidden_dim=64):
        ...
        # URL gates HTML
        self.url_to_html_gate = nn.Sequential(
        ...
        # HTML gates URL
        self.html_to_url_gate = nn.Sequential(
            nn.Linear(hidden_dim * 2 + 32, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.Sigmoid()
        ...
        self.interaction = nn.Sequential(
            nn.Linear(hidden_dim * 4 + 32, 64),
            nn.ReLU(),
            nn.Dropout(0.15)
        )

        self.classifier = nn.Sequential(
            nn.Linear(32 + hidden_dim * 4 + 64, 128),
            nn.ReLU(),
            nn.Dropout(0.30),
            nn.Linear(128, 32),
            nn.ReLU(),
    ...
    def forward(self, x_tab, x_url, x_html):
        ...
        html_gate = self.url_to_html_gate(torch.cat([u, h, t], dim=1))
        url_gate = self.html_to_url_gate(torch.cat([h, u, t], dim=1))

        h_gated = h * html_gate
        u_gated = u * url_gate

        inter = self.interaction(torch.cat([
            u, h, u_gated, h_gated, t
        ], dim=1))

        z = torch.cat([t, u, h, u_gated, h_gated, inter], dim=1)
...
def main():
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        print("\n" + "=" * 70)
        print(f"PHASE 04 CROSS-GATED FUSION FOLD {fold}/{N_SPLITS}")
        print("=" * 70)
        ...
        ds_train = CrossGatedDataset(X_tab_train[tr_idx], X_url_train[tr_idx], X_html_train[tr_idx], y_train[tr_idx])
        ds_val = CrossGatedDataset(X_tab_train[val_idx], X_url_train[val_idx], X_html_train[val_idx], y_train[val_idx])
        ds_test = CrossGatedDataset(X_tab_test, X_url_test, X_html_test, y_test)

        dl_train = DataLoader(ds_train, batch_size=BATCH_SIZE, shuffle=True)
        dl_val = DataLoader(ds_val, batch_size=BATCH_SIZE, shuffle=False)
        dl_test = DataLoader(ds_test, batch_size=BATCH_SIZE, shuffle=False)

        model = CrossGatedFusion(
        ...
        pred_df["cross_gated_prob"] = y_test_prob
        pred_df["cross_gated_pred"] = (y_test_prob >= 0.5).astype(int)
        pred_df["fold"] = fold
    ...
    oof_df.to_csv(OUT_DIR / "phase04_cross_gated_oof_predictions.csv", index=False)

    fold_metrics_df = pd.DataFrame(fold_metrics)
    fold_metrics_df.to_csv(OUT_DIR / "phase04_cross_gated_fold_metrics.csv", index=False)

    summary = fold_metrics_df.drop(columns=["Fold"]).agg(["mean", "std"]).T
    summary.to_csv(OUT_DIR / "phase04_cross_gated_summary.csv")

    print("\n" + "=" * 70)
    print("PHASE 04 CROSS-GATED FUSION SUMMARY")
    print("=" * 70)
    print(summary)

    y_true = oof_df["y_true"].astype(int).values
    y_prob = oof_df["cross_gated_prob"].astype(float).values
    sweep = build_threshold_sweep(y_true, y_prob)
    sweep.to_csv(OUT_DIR / "phase04_cross_gated_threshold_sweep.csv", index=False)
    ...
    with open(OUT_DIR / "phase04_cross_gated_threshold_summary.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("\nSaved:")
    print(OUT_DIR / "phase04_cross_gated_oof_predictions.csv")
    print(OUT_DIR / "phase04_cross_gated_fold_metrics.csv")
    print(OUT_DIR / "phase04_cross_gated_summary.csv")
    print(OUT_DIR / "phase04_cross_gated_threshold_sweep.csv")
