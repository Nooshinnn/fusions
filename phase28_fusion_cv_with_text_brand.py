...
TEXTBRAND_DIR = Path("phase27_text_brand_cv")
OUT_DIR = Path("phase28_fusion_cv_with_text_brand_outputs")
...
class FusionDataset(Dataset):
    def __init__(self, x_tab, x_html, x_brand, x_text, y):
        self.x_tab = x_tab.astype(np.float32)
        self.x_html = x_html.astype(np.float32)
        self.x_brand = x_brand.astype(np.float32)
        self.x_text = x_text.astype(np.float32)
    ...
    def __getitem__(self, i):
        return (
            ...
            torch.tensor(self.x_brand[i], dtype=torch.float32),
            torch.tensor(self.x_text[i], dtype=torch.float32),
            torch.tensor(self.y[i], dtype=torch.float32),
        )

class TextBrandFusion(nn.Module):
    def __init__(self, tab_dim, html_dim, brand_dim, text_dim):
        super().__init__()
        ...
        self.brand_proj = nn.Sequential(
            nn.Linear(brand_dim, 64),
            nn.ReLU(),
            nn.Dropout(0.15),
            nn.Linear(64, 32),
            nn.ReLU()
    ...
    def forward(self, x_tab, x_html, x_brand, x_text):
        t = self.tab_proj(x_tab)
        h = self.html_proj(x_html)
        b = self.brand_proj(x_brand)
        tx = self.text_proj(x_text)

        gate = self.gate(torch.cat([t, h, b, tx], dim=1))
        h_gated = h * (1.0 + gate)

        z = torch.cat([t, h, b, tx, h_gated], dim=1)
...
def train_epoch(model, loader, optimizer, criterion):
    model.train()
    total = 0.0
    for x_tab, x_html, x_brand, x_text, y in loader:
        ...
        logits = model(x_tab, x_html, x_brand, x_text)
...
def main():
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        tb_dir = TEXTBRAND_DIR / f"fold_{fold:02d}"

        print("\n" + "=" * 70)
        print(f"TEXT+BRAND FUSION FOLD {fold}/{N_SPLITS}")
        print("=" * 70)
        ...
        train_brand = np.load(tb_dir / "train_brand_features.npy").astype(np.float32)
        test_brand = np.load(tb_dir / "test_brand_features.npy").astype(np.float32)

        train_text_sparse = sparse.load_npz(tb_dir / "train_text_tfidf.npz")
        test_text_sparse = sparse.load_npz(tb_dir / "test_text_tfidf.npz")

        # reduce text dimension a bit by using top columns directly after MaxAbs scaling
        text_scaler = MaxAbsScaler()
        Xtext_train = text_scaler.fit_transform(train_text_sparse).astype(np.float32).toarray()
        Xtext_test = text_scaler.transform(test_text_sparse).astype(np.float32).toarray()

        # keep only first 512 columns if larger
        if Xtext_train.shape[1] > 512:
            Xtext_train = Xtext_train[:, :512]
            Xtext_test = Xtext_test[:, :512]
        ...
        Xbrand_train = scaler_brand.fit_transform(train_brand)
        Xbrand_test = scaler_brand.transform(test_brand)
        ...
        ds_train = FusionDataset(
            Xtab_train[tr_idx], Xhtml_train[tr_idx], Xbrand_train[tr_idx], Xtext_train[tr_idx], y_train[tr_idx]
        )
        ds_val = FusionDataset(
            Xtab_train[val_idx], Xhtml_train[val_idx], Xbrand_train[val_idx], Xtext_train[val_idx], y_train[val_idx]
        )
        ds_test = FusionDataset(
            Xtab_test, Xhtml_test, Xbrand_test, Xtext_test, y_test
        )
        ...
        model = TextBrandFusion(
            tab_dim=Xtab_train.shape[1],
            html_dim=Xhtml_train.shape[1],
            brand_dim=Xbrand_train.shape[1],
            text_dim=Xtext_train.shape[1]
        ).to(DEVICE)
    ...
    oof_df.to_csv(OUT_DIR / "phase28_text_brand_fusion_oof_predictions.csv", index=False)

    metrics_df = pd.DataFrame(all_fold_metrics)
    metrics_df.to_csv(OUT_DIR / "phase28_text_brand_fold_metrics.csv", index=False)

    summary = metrics_df.drop(columns=["Fold"]).agg(["mean", "std"]).T
    summary.to_csv(OUT_DIR / "phase28_text_brand_metrics_summary.csv")

    print("\n" + "=" * 70)
    print("PHASE 28 TEXT+BRAND SUMMARY")
    print("=" * 70)
    print(summary)

    print("\nSaved:")
    print(OUT_DIR / "phase28_text_brand_fusion_oof_predictions.csv")
    print(OUT_DIR / "phase28_text_brand_fold_metrics.csv")
    print(OUT_DIR / "phase28_text_brand_metrics_summary.csv")
