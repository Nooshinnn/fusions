...
OUT_DIR = Path("phase10_5_outputs")
...
def load_and_merge():
    ...
    if not np.array_equal(html_df["url"].astype(str).values, url_df["url"].astype(str).values):
        raise ValueError("url mismatch between HTML and URL files")
    ...
    # new stable confidence features
    fusion_df["std_prob"] = fusion_df[["html_prob", "url_textcnn_prob", "url_rf_prob"]].std(axis=1)
    fusion_df["html_conf"] = np.abs(fusion_df["html_prob"] - 0.5)
    fusion_df["url_conf"] = np.abs(fusion_df["url_textcnn_prob"] - 0.5)
    fusion_df["rf_conf"] = np.abs(fusion_df["url_rf_prob"] - 0.5)

    fusion_df["html_gt_url"] = (fusion_df["html_prob"] > fusion_df["url_textcnn_prob"]).astype(float)
    fusion_df["url_gt_html"] = (fusion_df["url_textcnn_prob"] > fusion_df["html_prob"]).astype(float)
    fusion_df["rf_gt_html"] = (fusion_df["url_rf_prob"] > fusion_df["html_prob"]).astype(float)

    fusion_df["top_gap"] = fusion_df["max_prob"] - fusion_df["mean_prob"]
...
class GatedFusionMLP(nn.Module):
    ...
    def forward(self, x_tab, x_html):
        ...
        gate = self.gate(torch.cat([t, h], dim=1))
        h_gated = gate * h
...
def run_gated_mlp(X_tab, X_html, y):
    ...
    for fold, (tr_idx, te_idx) in enumerate(skf.split(X_tab, y), 1):
        ...
        ds_train = FusionMLPDataset(
            Xtab_tr[inner_train_idx], Xhtml_tr[inner_train_idx], y[tr_idx][inner_train_idx]
        )
        ds_val = FusionMLPDataset(
            Xtab_tr[val_idx], Xhtml_tr[val_idx], y[tr_idx][val_idx]
        )
        ds_test = FusionMLPDataset(
            Xtab_te, Xhtml_te, y[te_idx]
        )
...
def main():
    ...
    # stronger stable tabular fusion inputs
    tab_cols = [
        "html_prob",
        "url_textcnn_prob",
        "url_rf_prob",
    ...
    fusion_df.to_csv(OUT_DIR / "phase10_5_fusion_input_table.csv", index=False)
    ...
    # save oof table
    out = fusion_df[["row_index", "url", "label", "y_true"]].copy()
    out["lr_fusion_prob"] = lr_oof
    out["mlp_fusion_prob"] = mlp_oof

    best_thr = mlp_metrics["Threshold"]
    out["mlp_fusion_pred"] = (mlp_oof >= best_thr).astype(int)

    out.to_csv(OUT_DIR / "phase10_5_fusion_oof_predictions.csv", index=False)

    # FP / FN
    fps = out[(out["y_true"] == 0) & (out["mlp_fusion_pred"] == 1)].copy()
    fns = out[(out["y_true"] == 1) & (out["mlp_fusion_pred"] == 0)].copy()

    fps.to_csv(OUT_DIR / "phase10_5_false_positives.csv", index=False)
    fns.to_csv(OUT_DIR / "phase10_5_false_negatives.csv", index=False)

    # threshold sweep on final MLP OOF probs
    sweep_df = build_threshold_sweep(y, mlp_oof)
    sweep_df.to_csv(OUT_DIR / "phase10_5_threshold_sweep.csv", index=False)
    ...
    if best_low_fn is not None:
        ...
        print(f"\nNo threshold satisfied precision >= {PRECISION_FLOOR:.2f}")

    print("\nFive false negative samples:")
    print(fns.head(5).to_string(index=False))

    with open(OUT_DIR / "phase10_5_metrics_summary.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "tab_feature_dim": int(X_tab.shape[1]),
            "html_feature_dim": int(X_html.shape[1]),
            "fusion_rows": int(len(fusion_df)),
        }, f, indent=2)

    print("\nSaved:")
    print(OUT_DIR / "phase10_5_fusion_input_table.csv")
    print(OUT_DIR / "phase10_5_fusion_oof_predictions.csv")
    print(OUT_DIR / "phase10_5_false_positives.csv")
    print(OUT_DIR / "phase10_5_false_negatives.csv")
    print(OUT_DIR / "phase10_5_threshold_sweep.csv")
    print(OUT_DIR / "phase10_5_metrics_summary.json")
