...
DOMAIN_DIR = Path("phase30_domain_intel_folds")
OUT_DIR = Path("phase31_fusion_cv_with_domain_intel_outputs")
...
class FusionDataset(Dataset):
    def __init__(self, x_tab, x_html, x_dom, y):
        self.x_tab = x_tab.astype(np.float32)
        self.x_html = x_html.astype(np.float32)
        self.x_dom = x_dom.astype(np.float32)
    ...
    def __getitem__(self, i):
        return (
            ...
            torch.tensor(self.x_dom[i], dtype=torch.float32),
            torch.tensor(self.y[i], dtype=torch.float32),
        )

class DomainIntelFusion(nn.Module):
    def __init__(self, tab_dim, html_dim, dom_dim):
        super().__init__()
    ...
    def forward(self, x_tab, x_html, x_dom):
        t = self.tab_proj(x_tab)
        h = self.html_proj(x_html)
        d = self.dom_proj(x_dom)

        gate = self.gate(torch.cat([t, h, d], dim=1))
        h_gated = h * (1.0 + gate)

        z = torch.cat([t, h, d, h_gated], dim=1)
...
def train_epoch(model, loader, optimizer, criterion):
    model.train()
    total = 0.0
    for x_tab, x_html, x_dom, y in loader:
        ...
        logits = model(x_tab, x_html, x_dom)
...
def main():
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        print("\n" + "=" * 70)
        print(f"DOMAIN-INTEL FUSION FOLD {fold}/{N_SPLITS}")
        print("=" * 70)
        ...
        train_dom = np.load(dom_dir / "train_domain_intel.npy").astype(np.float32)
        test_dom = np.load(dom_dir / "test_domain_intel.npy").astype(np.float32)
        ...
        Xdom_train = scaler_dom.fit_transform(train_dom)
        Xdom_test = scaler_dom.transform(test_dom)
        ...
        ds_train = FusionDataset(Xtab_train[tr_idx], Xhtml_train[tr_idx], Xdom_train[tr_idx], y_train[tr_idx])
        ds_val = FusionDataset(Xtab_train[val_idx], Xhtml_train[val_idx], Xdom_train[val_idx], y_train[val_idx])
        ds_test = FusionDataset(Xtab_test, Xhtml_test, Xdom_test, y_test)

        dl_train = DataLoader(ds_train, batch_size=BATCH_SIZE, shuffle=True)
        dl_val = DataLoader(ds_val, batch_size=BATCH_SIZE, shuffle=False)
        dl_test = DataLoader(ds_test, batch_size=BATCH_SIZE, shuffle=False)

        model = DomainIntelFusion(
            tab_dim=Xtab_train.shape[1],
            html_dim=Xhtml_train.shape[1],
            dom_dim=Xdom_train.shape[1]
        ).to(DEVICE)
    ...
    oof_df.to_csv(OUT_DIR / "phase31_domain_intel_fusion_oof_predictions.csv", index=False)

    metrics_df = pd.DataFrame(all_fold_metrics)
    metrics_df.to_csv(OUT_DIR / "phase31_domain_intel_fold_metrics.csv", index=False)

    summary = metrics_df.drop(columns=["Fold"]).agg(["mean", "std"]).T
    summary.to_csv(OUT_DIR / "phase31_domain_intel_metrics_summary.csv")

    print("\n" + "=" * 70)
    print("PHASE 31 DOMAIN-INTEL SUMMARY")
    print("=" * 70)
    print(summary)

    print("\nSaved:")
    print(OUT_DIR / "phase31_domain_intel_fusion_oof_predictions.csv")
    print(OUT_DIR / "phase31_domain_intel_fold_metrics.csv")
    print(OUT_DIR / "phase31_domain_intel_metrics_summary.csv")
