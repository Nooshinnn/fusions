...
OUT_DIR = Path("phase03_hybrid_fusion_outputs")
...
def main():
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        print("\n" + "=" * 70)
        print(f"PHASE 03 HYBRID FUSION FOLD {fold}/{N_SPLITS}")
        print("=" * 70)
        ...
        ds_train = HybridDataset(X_tab_train[tr_idx], X_url_train[tr_idx], X_html_train[tr_idx], y_train[tr_idx])
        ds_val = HybridDataset(X_tab_train[val_idx], X_url_train[val_idx], X_html_train[val_idx], y_train[val_idx])
        ds_test = HybridDataset(X_tab_test, X_url_test, X_html_test, y_test)

        dl_train = DataLoader(ds_train, batch_size=BATCH_SIZE, shuffle=True)
        dl_val = DataLoader(ds_val, batch_size=BATCH_SIZE, shuffle=False)
        dl_test = DataLoader(ds_test, batch_size=BATCH_SIZE, shuffle=False)

        model = HybridFusionMLP(
        ...
        pred_df["hybrid_prob"] = y_test_prob
        pred_df["hybrid_pred"] = (y_test_prob >= 0.5).astype(int)
        pred_df["fold"] = fold
    ...
    oof_df.to_csv(OUT_DIR / "phase03_hybrid_oof_predictions.csv", index=False)

    fold_metrics_df = pd.DataFrame(fold_metrics)
    fold_metrics_df.to_csv(OUT_DIR / "phase03_hybrid_fold_metrics.csv", index=False)

    summary = fold_metrics_df.drop(columns=["Fold"]).agg(["mean", "std"]).T
    summary.to_csv(OUT_DIR / "phase03_hybrid_summary.csv")

    print("\n" + "=" * 70)
    print("PHASE 03 HYBRID FUSION SUMMARY")
    print("=" * 70)
    print(summary)

    y_true = oof_df["y_true"].astype(int).values
    y_prob = oof_df["hybrid_prob"].astype(float).values
    sweep = build_threshold_sweep(y_true, y_prob)
    sweep.to_csv(OUT_DIR / "phase03_hybrid_threshold_sweep.csv", index=False)
    ...
    with open(OUT_DIR / "phase03_hybrid_threshold_summary.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("\nSaved:")
    print(OUT_DIR / "phase03_hybrid_oof_predictions.csv")
    print(OUT_DIR / "phase03_hybrid_fold_metrics.csv")
    print(OUT_DIR / "phase03_hybrid_summary.csv")
    print(OUT_DIR / "phase03_hybrid_threshold_sweep.csv")
