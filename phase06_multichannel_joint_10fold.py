...
OUT_DIR = Path("phase06_multichannel_joint_outputs")
...
# MODEL
# ============================================================
class MultiChannelJointModel(nn.Module):
    def __init__(self, tab_dim, url_dim, html_dim, hidden_dim=64):
        ...
        # channel 1: URL branch
        self.url_channel = nn.Sequential(
        ...
        # channel 2: HTML branch
        self.html_channel = nn.Sequential(
    ...
    def forward(self, x_tab, x_url, x_html):
        ...
        u = self.url_channel(x_url)
        h = self.html_channel(x_html)

        mix_in = torch.cat([u, h, torch.abs(u - h), u * h], dim=1)
        m = self.channel_mix(mix_in)
...
def main():
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        print("\n" + "=" * 70)
        print(f"PHASE 06 MULTI-CHANNEL JOINT FOLD {fold}/{N_SPLITS}")
        print("=" * 70)
        ...
        ds_train = MultiChannelDataset(X_tab_train[tr_idx], X_url_train[tr_idx], X_html_train[tr_idx], y_train[tr_idx])
        ds_val = MultiChannelDataset(X_tab_train[val_idx], X_url_train[val_idx], X_html_train[val_idx], y_train[val_idx])
        ds_test = MultiChannelDataset(X_tab_test, X_url_test, X_html_test, y_test)

        dl_train = DataLoader(ds_train, batch_size=BATCH_SIZE, shuffle=True)
        dl_val = DataLoader(ds_val, batch_size=BATCH_SIZE, shuffle=False)
        dl_test = DataLoader(ds_test, batch_size=BATCH_SIZE, shuffle=False)

        model = MultiChannelJointModel(
        ...
        pred_df["multichannel_prob"] = y_test_prob
        pred_df["multichannel_pred"] = (y_test_prob >= 0.5).astype(int)
        pred_df["fold"] = fold
    ...
    oof_df.to_csv(OUT_DIR / "phase06_multichannel_oof_predictions.csv", index=False)

    fold_metrics_df = pd.DataFrame(fold_metrics)
    fold_metrics_df.to_csv(OUT_DIR / "phase06_multichannel_fold_metrics.csv", index=False)

    summary = fold_metrics_df.drop(columns=["Fold"]).agg(["mean", "std"]).T
    summary.to_csv(OUT_DIR / "phase06_multichannel_summary.csv")

    print("\n" + "=" * 70)
    print("PHASE 06 MULTI-CHANNEL JOINT SUMMARY")
    print("=" * 70)
    print(summary)

    y_true = oof_df["y_true"].astype(int).values
    y_prob = oof_df["multichannel_prob"].astype(float).values
    sweep = build_threshold_sweep(y_true, y_prob)
    sweep.to_csv(OUT_DIR / "phase06_multichannel_threshold_sweep.csv", index=False)
    ...
    with open(OUT_DIR / "phase06_multichannel_threshold_summary.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("\nSaved:")
    print(OUT_DIR / "phase06_multichannel_oof_predictions.csv")
    print(OUT_DIR / "phase06_multichannel_fold_metrics.csv")
    print(OUT_DIR / "phase06_multichannel_summary.csv")
    print(OUT_DIR / "phase06_multichannel_threshold_sweep.csv")
