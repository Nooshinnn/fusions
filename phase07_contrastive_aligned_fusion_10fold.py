...
OUT_DIR = Path("phase07_contrastive_aligned_outputs")
...
class ContrastiveAlignedFusion(nn.Module):
    def __init__(self, tab_dim, url_dim, html_dim, hidden_dim=64):
        super().__init__()
        ...
        # alignment heads
        self.url_align = nn.Linear(hidden_dim, hidden_dim)
        self.html_align = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, x_tab, x_url, x_html):
        ...
        u_a = F.normalize(self.url_align(u), dim=1)
        h_a = F.normalize(self.html_align(h), dim=1)

        mix_in = torch.cat([u, h, torch.abs(u - h), u * h], dim=1)
        m = self.mix(mix_in)

        z = torch.cat([t, u, h, m], dim=1)
        logits = self.classifier(z).squeeze(1)

        return logits, u_a, h_a

# ============================================================
# CONTRASTIVE LOSS
# ============================================================
def symmetric_nt_xent(u, h, temperature=0.2):
    logits = (u @ h.T) / temperature
    labels = torch.arange(u.size(0), device=u.device)
    loss_u2h = F.cross_entropy(logits, labels)
    loss_h2u = F.cross_entropy(logits.T, labels)
    return 0.5 * (loss_u2h + loss_h2u)

# ============================================================
# TRAIN / PREDICT
# ============================================================
def train_epoch(model, loader, optimizer, criterion, align_weight, temperature):
    model.train()
    total = 0.0
    for x_tab, x_url, x_html, y in loader:
        ...
        logits, u_a, h_a = model(x_tab, x_url, x_html)
        cls_loss = criterion(logits, y)
        align_loss = symmetric_nt_xent(u_a, h_a, temperature=temperature)
...
def predict_probs(model, loader):
    ...
    for x_tab, x_url, x_html, y in loader:
        ...
        logits, _, _ = model(x_tab, x_url, x_html)
...
def main():
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        print("\n" + "=" * 70)
        print(f"PHASE 07 CONTRASTIVE ALIGNED FUSION FOLD {fold}/{N_SPLITS}")
        print("=" * 70)
        ...
        ds_train = ContrastiveDataset(X_tab_train[tr_idx], X_url_train[tr_idx], X_html_train[tr_idx], y_train[tr_idx])
        ds_val = ContrastiveDataset(X_tab_train[val_idx], X_url_train[val_idx], X_html_train[val_idx], y_train[val_idx])
        ds_test = ContrastiveDataset(X_tab_test, X_url_test, X_html_test, y_test)

        dl_train = DataLoader(ds_train, batch_size=BATCH_SIZE, shuffle=True, drop_last=True)
        dl_val = DataLoader(ds_val, batch_size=BATCH_SIZE, shuffle=False)
        dl_test = DataLoader(ds_test, batch_size=BATCH_SIZE, shuffle=False)

        model = ContrastiveAlignedFusion(
        ...
        for epoch in range(1, EPOCHS + 1):
            loss = train_epoch(model, dl_train, optimizer, criterion, ALIGN_WEIGHT, TEMPERATURE)
        ...
        pred_df["contrastive_prob"] = y_test_prob
        pred_df["contrastive_pred"] = (y_test_prob >= 0.5).astype(int)
        pred_df["fold"] = fold
    ...
    oof_df.to_csv(OUT_DIR / "phase07_contrastive_oof_predictions.csv", index=False)

    fold_metrics_df = pd.DataFrame(fold_metrics)
    fold_metrics_df.to_csv(OUT_DIR / "phase07_contrastive_fold_metrics.csv", index=False)

    summary = fold_metrics_df.drop(columns=["Fold"]).agg(["mean", "std"]).T
    summary.to_csv(OUT_DIR / "phase07_contrastive_summary.csv")

    print("\n" + "=" * 70)
    print("PHASE 07 CONTRASTIVE ALIGNED FUSION SUMMARY")
    print("=" * 70)
    print(summary)

    y_true = oof_df["y_true"].astype(int).values
    y_prob = oof_df["contrastive_prob"].astype(float).values
    sweep = build_threshold_sweep(y_true, y_prob)
    sweep.to_csv(OUT_DIR / "phase07_contrastive_threshold_sweep.csv", index=False)
    ...
    with open(OUT_DIR / "phase07_contrastive_threshold_summary.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("\nSaved:")
    print(OUT_DIR / "phase07_contrastive_oof_predictions.csv")
    print(OUT_DIR / "phase07_contrastive_fold_metrics.csv")
    print(OUT_DIR / "phase07_contrastive_summary.csv")
    print(OUT_DIR / "phase07_contrastive_threshold_sweep.csv")
